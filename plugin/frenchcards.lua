...
local cards = {
    ...
    { q = "They do not care about it", a = "Ils s'en foutent", meta = "masc" },    { q = "I realized it", a = "Je m'en suis rendu compte", meta = "masc" },
...
local function check_answer()
    ...
    -- Read the last line of the buffer where the user types the answer
    local line_count = vim.api.nvim_buf_line_count(buf)
    local lines = vim.api.nvim_buf_get_lines(buf, line_count - 1, line_count, false)
    local user_answer = lines[1] or ""

    -- Trim whitespace
    user_answer = user_answer:match("^%s*(.-)%s*$") or ""

    if user_answer:lower() == current_card.a:lower() then
        vim.api.nvim_echo({{ "Correct! The answer is: " .. current_card.a, "Title" }}, false, {})
    else
...
local function show_answer()
    if not buf or not vim.api.nvim_buf_is_valid(buf) or not current_card then return end
    vim.api.nvim_echo({{ "The correct answer is: " .. current_card.a, "WarningMsg" }}, false, {})
end

local function open_flashcard()
    ...
    -- Randomly flip question and answer to practice both directions
    if math.random() > 0.5 then
        current_card = { q = card.a, a = card.q, meta = card.meta }
    else
        current_card = { q = card.q, a = card.a, meta = card.meta }
    end

    -- Create a new regular buffer instead of floating
    vim.cmd("enew")
    ...
    local meta_line = current_card.meta and ("[" .. current_card.meta .. "]") or ""
    local content = {
        "",
        current_card.q,
        meta_line,
    ...
    -- Map <CR> to check the answer in normal and insert mode
    vim.keymap.set({ 'n', 'i' }, '<CR>', function()
        vim.cmd("stopinsert")
        vim.schedule(check_answer)
    end, { buffer = buf })

    -- Map \\ to show the answer (normal mode only)
    vim.keymap.set('n', '\\\\', function()
        show_answer()
    end, { buffer = buf })
end

vim.api.nvim_create_user_command("French", open_flashcard, {})
